...
export class BrowserViewWindow extends EventEmitter.EventEmitter2 {
  ...
  public async launch(startUrl?: string) {
    ...
    const { disableToolBarList } = this.config;
    this._panel.webview.html = disableToolBarList.includes(startUrl as string) || startUrl==='https://weread.qq.com'
      ? this.contentProvider.getContent(true)
    ...
    // Update startUrl if requested to launch specific page.
    if (startUrl) {
